...
const categories = [
  { key: "agent", label: "Agent Threats" },
  { key: "conventional", label: "Conventional" },
  { key: "all", label: "All" },
];

// Type chips shown per category; agent-first, human-readable via typeLabel.
const typeFiltersByCategory: Record<string, string[]> = {
  agent: ["all", "prompt-injection", "tool-poisoning", "memory-poisoning", "agent-worm", "framework-vulnerability", "protocol-vulnerability"],
  conventional: ["all", "ransomware", "apt", "zero-day", "supply-chain", "phishing"],
  all: ["all"],
};
...
export default function LibraryContent() {
  ...
  const [category, setCategory] = useState(params.get("category") || "agent");
  ...
  useEffect(() => { fetchThreats(); }, [page, category, typeFilter, sevFilter]);
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Agent-to-agent threats first — conventional coverage one click away.</p>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-1.5 pl-5 flex gap-2 shadow-sm focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-100 transition-all">
        <input
          ...
          placeholder="Search threats... e.g. 'MCP tool poisoning' or 'CVE-2025'"
      ...
      <div className="flex gap-2 flex-wrap items-center">
        {categories.map((c) => (
          <button key={c.key} onClick={() => switchCategory(c.key)}
            className={`px-4 py-1.5 rounded-lg text-xs font-mono transition-all border ${category === c.key ? "bg-purple-50 border-purple-500 text-purple-600 font-semibold" : "bg-white border-gray-200 text-gray-500 hover:border-purple-300"}`}>
            {c.label}
          </button>
        ))}
      </div>
              ...
              <div className="flex-1">
                ...
                <div className="flex gap-1.5 mb-2 flex-wrap">
                  {t.tags.split(",").filter(Boolean).map((tag) => (
                    <span key={tag} className="text-[10px] font-mono px-2 py-0.5 bg-gray-100 rounded text-gray-500">{formatTag(tag)}</span>
                  ))}
                </div>
