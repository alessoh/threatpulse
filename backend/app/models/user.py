...
class DailyInsight(Base):
    """One AI-written landscape summary per UTC day, generated by Gemini from
    the week's threats and shown in the dashboard's AI Insight box."""

    __tablename__ = "daily_insights"

    id = Column(Integer, primary_key=True, index=True)
    date = Column(String(10), unique=True, index=True, nullable=False)  # YYYY-MM-DD (UTC)
